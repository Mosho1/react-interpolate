...
export const describeAsync = async (label, fn) => {
	console.group(label);
	await fn();
	console.groupEnd(label);
};
...
export const it = async (label, fn, isAsync) => {
	doTest(fn);
	postIt(label);
};

export const itAsync = async (label, fn, isAsync) => {
	await doTestAsync(fn);
	postIt(label);
};
